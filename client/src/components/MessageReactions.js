...
const MessageReactions = ({ reactions = [], onAddReaction, onRemoveReaction, currentUserId }) => {
  ...
  const handleReactionClick = emoji => {
    const hasReacted = reactions
      ...
      ?.users.some(userId => userId === currentUserId);
  ...
  const hasUserReacted = emoji => {
    return reactions.find(r => r.emoji === emoji)?.users.some(userId => userId === currentUserId);
  };
    ...
    <>
      <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
        {Object.entries(groupedReactions).map(([emoji, users]) => (
          <motion.div key={emoji} initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}>
            <Chip
              label={
              ...
              sx={{
                cursor: 'pointer',
                '&:hover': {
                  bgcolor: hasUserReacted(emoji) ? 'primary.dark' : 'action.hover',
                },
              }}
            />
          </motion.div>
      ...
      >
        {showPicker ? (
          <Box>
            ...
            <Picker data={data} onEmojiSelect={handleEmojiSelect} theme='dark' set='native' />
          </Box>
        ) : (
          <Box sx={{ display: 'flex', gap: 0.5 }}>
            {QUICK_REACTIONS.map(emoji => (
              <IconButton key={emoji} size='small' onClick={() => handleQuickReaction(emoji)}>
                {emoji}
              </IconButton>
            ))}
        ...
        <List dense>
          <ListItem>
            ...
            <ListItemText
              ...
              secondary={`${getReactionCount(selectedEmoji)} ${
                getReactionCount(selectedEmoji) === 1 ? 'person' : 'people'
              }`}
            />
          </ListItem>
          <Divider />
          {getReactors(selectedEmoji).map(user => (
            <ListItem key={user.id}>
              <ListItemAvatar>
                <Avatar src={user.avatar?.url}>{user.username[0].toUpperCase()}</Avatar>
              </ListItemAvatar>
              <ListItemText
