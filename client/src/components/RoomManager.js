          ...
          >
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                <Avatar src={room.avatar?.url}>{room.name[0].toUpperCase()}</Avatar>
              ...
              <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                {room.categories.map(category => (
                  <Chip key={category} icon={<CategoryIcon />} label={category} size='small' />
                ))}
                {room.tags.map(tag => (
                  <Chip key={tag} icon={<TagIcon />} label={tag} size='small' variant='outlined' />
                ))}
              </Box>
            </Box>
          </motion.div>
