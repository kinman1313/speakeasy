      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
          ...
          <Box sx={{ flexGrow: 1 }}>
            ...
            <MessageBubble message={message} isOwn={message?.user?.id === currentUser?.id} />
            <Box sx={{ mt: 1, mb: 1 }}>
              <MessageReactions
          ...
          {showReplyInput && (
            ...
            >
              <Box sx={{ mt: 2, display: 'flex', gap: 2 }}>
                <Avatar src={currentUser?.profile?.avatar?.url} sx={{ width: 32, height: 32 }}>
                ...
                <IconButton size='small' onClick={() => setShowReplyInput(false)}>
                  <CloseIcon />
                </IconButton>
              </Box>
            </motion.div>
        ...
        <Collapse in={expanded}>
          {replies.length > 0 && (
            <Box sx={{ mt: 2, ml: 6 }}>
                ...
                {replies.map((reply, index) => (
                  <React.Fragment key={reply.id}>
                    <ListItem alignItems='flex-start' sx={{ px: 0 }}>
                      <ListItemAvatar>
                        <Avatar src={reply?.user?.avatar?.url} sx={{ width: 32, height: 32 }}>
              ...
              {hasMoreReplies && (
                <Button fullWidth onClick={onLoadMore} disabled={isLoadingReplies} sx={{ mt: 1 }}>
