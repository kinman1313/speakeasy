...
export const MessageProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    socket.on('messageUpdate', ({ messageId, update }) => {
      setMessages(prev => prev.map(msg => (msg.id === messageId ? { ...msg, ...update } : msg)));
    });

    // Listen for voice message status updates
    socket.on('voiceMessageStatus', ({ messageId, status, url }) => {
      setMessages(prev =>
        prev.map(msg => (msg.id === messageId ? { ...msg, status, content: url } : msg))
      );
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.emit('sendMessage', messageData, response => {
        if (response.success) {
          ...
          setMessages(prev =>
            prev.map(msg => (msg.id === tempId ? { ...msg, ...response.message } : msg))
          );
